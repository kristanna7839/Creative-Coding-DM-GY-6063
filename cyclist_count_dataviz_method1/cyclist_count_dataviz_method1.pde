...
void draw(){
  ...
  for( int ll = 0; ll < 28 ; ll++ ){ // Draw interface to show year on the left side
    strokeWeight(1);
    stroke(210);
    line(0,21*ll,width/2,21*ll); // The distance between each line is 21
    textSize(14);
    fill(10+(ll*10),10,10,35); // THE MORE RECENT YEAR, THE MORE RED
    text(data[ll][0],45,21*ll-5+21); 
  }
    line(0,21*28,width/2,21*28); // Draw the last line
  
 
  int yposition = 110; // for Adjust Graph Position
  ...
  for (int n = 0; n < 28 ; n++) {  // FOR 27 ROW
  
      if (  mouseY > 0+(n*21) && mouseY < 21+(n*21) ){   // Set Mouse Condition for each year
      
        
         for ( int a = 1 ; a < data[n].length ; a++) {
          int graphData = data[n][a]; // Get data from Array
          int graphHigh = graphData/30; // Calculate for graph's height
              ...
              fill(0);
              textSize(14); 
              text(headers[a],(width-leftMargin)-graphHigh-10,yposition+6); // Draw Grand Total text
              textSize(30); // SET TEXT SIZE FOR YEAR
              text(data[n][0],width-leftMargin+5,yposition + 50);  // Draw Year text
              }else{ 
              strokeWeight(4);
              stroke(150);
              fill(150);
              text(headers[a],(width-leftMargin)-graphHigh-10,yposition+3); // Draw other locations text
              }//end else    
          line (width-leftMargin,yposition,(width-leftMargin)-graphHigh,yposition);  // Draw graphs

          // ADDING SOME EFFECT
          noFill();
          strokeWeight(8);
          stroke(70+(n*10),70,70,20);  // THE MORE RECENT YEAR, THE MORE RED
          ellipse((width-leftMargin)-graphHigh+(random(-2,2)),yposition+(random(-2,2)),graphHigh/1.5,graphHigh/1.5);  // Draw Wheels
          
          yposition = yposition + 40 ; // Distance between each graph is 40
